...
self.addEventListener('activate', (event) => {
  event.waitUntil((async () => {
    ...
    await Promise.all(
      // 같은 출처의 홈 캐시(home-*)는 지우지 않는다 — 교인 앱 옛 캐시(gmc-*)만 정리
      keys.filter((k) => k.startsWith('gmc-') && !k.startsWith(CACHE_VERSION)).map((k) => caches.delete(k))
    );
